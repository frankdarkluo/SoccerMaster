...
class QWEN2_5VL_JN_AND_ROLE_BATCH(DetectionLevelModule):
    ...
    output_columns = ["jersey_number_detection", "jersey_number_confidence", "role_detection", "role_confidence"]
    ...
    def __init__(self, cfg, batch_size, device, tracking_dataset=None):
        ...
        self.processor = AutoProcessor.from_pretrained(self.model_path, max_pixels=int(cfg.image_width*cfg.image_height/cfg.downsample_factor/cfg.downsample_factor)//(28*28)*(28*28), use_fast=False)
        self.processor.tokenizer.padding_side = "left"
        self.batch_size = batch_size
        self.device = device
        
        # Jersey number detection prompt
        self.jn_text_prompt = "Analyze this image and determine if the player is facing away from the camera. If the player is facing away, output the jersey number on their back. If the player is not facing away from the camera, output 'No'."
    ...
    def preprocess(self, image, detection: pd.Series, metadata: pd.Series):
        ...
        batch = {
            'images': Unbatchable([image]),  # 全图用于 role 检测
            'crops': Unbatchable([crop])      # 裁剪图用于 jersey number 和 role 检测
        }
    ...
    def process(self, batch, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        if len(jn_idxs) > 0:
            ...
            messages = [[{"role": "user", "content":[{"type": "image", "image": Image.fromarray(crop)}, {"type": "text", "text": self.jn_text_prompt}]}] for crop in crops]
        ...
        # ============ Step 2: Role 检测 ============
        role_idxs = list(range(len(batch['crops'])))  # 所有样本都进行 role 检测
        
        if len(role_idxs) > 0:
            images = [batch['images'][idx].cpu().numpy() for idx in role_idxs]
            crops = [batch['crops'][idx].cpu().numpy() for idx in role_idxs]
            
            messages = [[{"role": "user", "content":[{"type": "image", "image": Image.fromarray(image)}, {"type": "image", "image": Image.fromarray(crop)}, {"type": "text", "text": self.role_text_prompt}]}] for image, crop in zip(images, crops)]
