...
def draw_pitch(patch, detections_pred, detections_gt,
    ...
    if detections_gt is not None and "bbox_pitch" in detections_gt:
        draw_radar_view(patch, detections_gt, scale=pitch_scale, group="ground truth", vis_pitch_corners=vis_pitch_corners, pitch_corners=None)
    if detections_pred is not None and "bbox_pitch" in detections_pred:
        if "pitch_corners" in image_pred:
            pitch_corners = image_pred["pitch_corners"]
        else:
            pitch_corners = None
        draw_radar_view(patch, detections_pred, scale=pitch_scale, group="predictions", vis_pitch_corners=vis_pitch_corners, pitch_corners=pitch_corners)

def draw_radar_view(patch, detections, scale, delta=32, group="ground truth", vis_pitch_corners=False, pitch_corners=None):
    ...
    if pitch_corners is not None:
        # pitch_corners is a list of 4 points，left_top, right_top, right_bottom, left_bottom
        # darw 4 lines
        # clip first
        # pitch_corners = [np.clip(corner, -10000, 10000) for corner in pitch_corners]
        pitch_corners = np.clip(pitch_corners, -10000, 10000)
        
        cv2.line(patch, (radar_center_x + int(pitch_corners[0, 0] * scale),
                         radar_center_y + int(pitch_corners[0, 1] * scale)),
                        (radar_center_x + int(pitch_corners[1, 0] * scale),
                        radar_center_y + int(pitch_corners[1, 1] * scale)),
                        color=(255, 218, 185),
                        thickness=2)
        cv2.line(patch, (radar_center_x + int(pitch_corners[1, 0] * scale),
                         radar_center_y + int(pitch_corners[1, 1] * scale)),
                        (radar_center_x + int(pitch_corners[2, 0] * scale),
                        radar_center_y + int(pitch_corners[2, 1] * scale)),
                        color=(255, 218, 185),
                        thickness=2)
        cv2.line(patch, (radar_center_x + int(pitch_corners[2, 0] * scale),
                         radar_center_y + int(pitch_corners[2, 1] * scale)),
                        (radar_center_x + int(pitch_corners[3, 0] * scale),
                        radar_center_y + int(pitch_corners[3, 1] * scale)),
                        color=(255, 218, 185),
                        thickness=2)
        cv2.line(patch, (radar_center_x + int(pitch_corners[3, 0] * scale),
                         radar_center_y + int(pitch_corners[3, 1] * scale)),
                        (radar_center_x + int(pitch_corners[0, 0] * scale),
                        radar_center_y + int(pitch_corners[0, 1] * scale)),
                        color=(255, 218, 185),
                        thickness=2)