...
class PitchVisualizationEngine(Callback):
    ...
    def _draw_detection(self, patch, detection, is_prediction):
        ...
        # display jersey number detection
        if (
                is_prediction
                and self.cfg.prediction.display_jersey_number_detection
                and hasattr(detection, "jersey_number_detection")
                and detection.role == "player"
        ):
            jn_d = str(int(detection.jersey_number_detection)) if not pd.isna(detection.jersey_number_detection) else "?"
            l, t, r, b = detection.bbox.ltrb(
                image_shape=(patch.shape[1], patch.shape[0]), rounded=True
            )
            text_size += draw_text(
        ...
        # display jersey number full detection
        if (
                is_prediction
                and self.cfg.prediction.display_jersey_number_full_detection
                and hasattr(detection, "jersey_number_full_detection")
                and detection.role == "player"
        ):
            jn_full = str(detection.jersey_number_full_detection) if str(detection.jersey_number_full_detection) != "" else "?"
            l, t, r, b = detection.bbox.ltrb(
                image_shape=(patch.shape[1], patch.shape[0]), rounded=True
            )
            text_size += draw_text(
