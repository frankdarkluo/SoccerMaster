...
class TrackerState(AbstractContextManager):
    def __init__(
            ...
            load_ball_from_groundtruth=False,
        ...
        self.load_ball_from_groundtruth = load_ball_from_groundtruth
    ...
    def load_groundtruth(self, load_columns):
        ...
        if self.pipeline.is_empty():
            detections_gt_copy = self.detections_gt.copy()
            if not self.load_ball_from_groundtruth:
                detections_gt_copy = detections_gt_copy[detections_gt_copy.role != 'ball']
            self.detections_pred_gt = detections_gt_copy  # load all columns if pipeline is empty
            ...
            if len(load_columns["detection"]) == 0:
                ...
                if not self.load_ball_from_groundtruth:
                    detections_gt_copy = detections_gt_copy[detections_gt_copy.role != 'ball']
                self.detections_pred_gt = detections_gt_copy[
    ...
    def on_video_loop_end(
        ...
        if detections is not None and image_pred is not None:
