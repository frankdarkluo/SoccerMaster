...
class TrackingEngine(ABC):
    ...
    def track_dataset(self):
        ...
        for i, (video_idx, video_metadata) in enumerate(
            ...
            with self.tracker_state(video_idx) as tracker_state:
                ...
                if self.is_video_in_results(video_idx):
                    continue
                    # detections, image_pred = None, None
                # else:
    ...
    def is_video_in_results(self, video_id: int) -> bool:
        if self.tracker_state.zf["save"] is not None:
            return f"{video_id}.pkl" in self.tracker_state.zf["save"].namelist()
        else:
            return False
