...
| `tactical_prototypes/recognition_evidence_auto.jsonl` | 3 | 这是本机有 GSR 输出的 reviewed claim 对应的纯轨迹自动证据；其余 claim 在一致率报告中记为 `no_data`。 |
...
| `tactical_prototypes/evidence_agreement_report.json` | 这是自动轨迹证据与手工证据的逐字段一致率、缺失类型、checker 判定及同子集回放对照。 |
...
| `tactical_prototypes/evidence_agreement_report.md` | 这是自动证据覆盖率、一致率、同子集精度对照和下一轮感知 backlog 的可读摘要。 |
